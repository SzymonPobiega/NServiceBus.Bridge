...
class SwitchImpl : ISwitch
{
    ...
    {
        await Task.WhenAll(ports.Values.Select(p => p.Initialize((ctx, infra, disp) => Forward(p.Name, ctx, infra, disp)))).ConfigureAwait(false);
        await Task.WhenAll(ports.Values.Select(p => p.StartReceiving())).ConfigureAwait(false);
    }

    Task Forward(string incomingPort, MessageContext msg, PubSubInfrastructure inboundPubSubInfrastructure, IDispatchMessages sourceDispatcher)
    {
        ...
        switch (intent)
        {
            ...
            case MessageIntentEnum.Send:
                ...
                return destinationPort.Forward(incomingPort, msg, inboundPubSubInfrastructure, sourceDispatcher);
            case MessageIntentEnum.Publish:
                return Task.WhenAll(ports.Values.Where(p => p.Name != incomingPort).Select(x => x.Forward(incomingPort, msg, inboundPubSubInfrastructure, sourceDispatcher)));
